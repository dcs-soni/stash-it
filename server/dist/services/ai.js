...
exports.addContentToVectorDB = addContentToVectorDB;
exports.removeFromVectorDB = removeFromVectorDB;
...
const chromaClient = new chromadb_1.ChromaClient({
    path: config_1.config.chroma.url
});
let collection;
let embedder;
let textGenerator;
...
async function initAI() {
    ...
    textGenerator = await (0, transformers_1.pipeline)("text-generation", "Xenova/gpt2");
}
...
async function addContentToVectorDB(id, content, metadata) {
    try {
        if (!collection) {
            await initAI();
        }
...
async function removeFromVectorDB(id) {
    try {
        if (!collection) {
            await initAI();
        }
...
async function searchContent(query, userId) {
    try {
        if (!collection) {
